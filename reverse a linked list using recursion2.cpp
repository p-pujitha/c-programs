...
struct node *reverse_a_linkedlist2(struct node *, struct node*, int , int);
...
void dotest()
{
		...
		{
			struct node *ptr= Array_to_linked_list(testcases[i].input,testcases[i].len);
			struct node *ptr1=reverse_a_linkedlist2(ptr,ptr,testcases[i].num1, testcases[i].num2);
			struct node *op= Array_to_linked_list(testcases[i].output,testcases[i].len);
			int ans= checkoutput(ptr1,op); 
     		 if(ans == 1)
				   printf("Testcase - %d -> valid\n",i+1);
...
struct node *reverse_a_linkedlist2(struct node *first, struct node *last, int num1, int num2)
{
	struct node *temp;
	...
	else
	{
		if (last != NULL)
		{
			...
			{
				if(last->data ==num1 ||last->data == num2);
				else{
				temp = last->next;
				last->next = first;
				first = last;
				last = temp;
				first = reverse_a_linkedlist2(first, last,num1,num2);}
			}
			return first;
		}
